...
// endpoints
app.MapGet("/", () => Results.Json(new { hello = "kRescue orchestrator", try = "/status" }));
app.MapGet("/status", () => Results.Json(new { ok = true, service = "orchestrator", version = "0.1.0" }));
...
app.MapPost("/backup", (BackupRequest? req) =>
{
    ...
    if (mode == "dry-run")
        return Results.Json(new BackupResponse(true, mode, name, DateTime.UtcNow));
    return Results.Json(new { ok = false, error = "live mode not implemented yet" });
});

app.Run();

// types below (must be after top-level statements)
public static class Helpers { public static string NewBackupName() => $"backup-{DateTime.UtcNow:yyyyMMdd-HHmmss}"; }
